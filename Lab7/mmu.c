    ...
    while (current != NULL) {
        ...
        if (size >= blocksize) {
            ...
            if (size > blocksize) {
                ...
                if (policy == 1) list_add_to_back(freelist, fragment);  // FIFO
                else if (policy == 2) list_add_ascending_by_blocksize(freelist, fragment);  // Best Fit
                else if (policy == 3) list_add_descending_by_blocksize(freelist, fragment);  // Worst Fit
            }
            free(current);
            return;
        }
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy) {
    ...
    while (current != NULL) {
        block_t *blk = current->blk;
        if (blk->pid == pid) {
            // Remove this block from allocated list
            if (prev) prev->next = current->next;
            else alloclist->head = current->next;

            blk->pid = 0;

            // Add back to free list based on policy
            if (policy == 1) list_add_to_back(freelist, blk);  // FIFO
            else if (policy == 2) list_add_ascending_by_blocksize(freelist, blk);  // Best Fit
            else if (policy == 3) list_add_descending_by_blocksize(freelist, blk);  // Worst Fit
